...
def train_owm(model, criterion, optimizer, pos_feats, neg_feats, maxiter, P4, P5, P6, W4, W5, W6, flag_old, in_layer='fc4'):
    model.train()

    if flag_old:
        for n, w in model.named_parameters():
            if n == 'layers.fc4.0.weight':
                w.data = torch.t(W4[:-1, :])

            if n == 'layers.fc4.0.bias':
                w.data = W4[-1, :]

            if n == 'layers.fc5.1.weight':
                w.data = torch.t(W5[:-1, :])

            if n == 'layers.fc5.1.bias':
                w.data = W5[-1, :]

            if n == 'branches.0.1.weight':
                w.data = torch.t(W6[:-1, :])

            if n == 'branches.0.1.bias':
                w.data = W6[-1, :]
        flag_old = 0
    ...
    for iter in range(maxiter):
        ...
        # forward
        #pos_score, h_list_pos = model.forward_owm(batch_pos_feats, in_layer=in_layer)
        #neg_score, h_list_neg = model.forward_owm(batch_neg_feats, in_layer=in_layer)

        score, h_list = model.forward_owm(torch.cat((batch_pos_feats, batch_neg_feats), dim=0), in_layer=in_layer)
        pos_score = score[:batch_pos_feats.size(0),:]
        neg_score = score[batch_pos_feats.size(0):,:]
        ...
        alpha_array = [1.0, 1.0, 1.0]

        def pro_weight(p, x, w, w_bk, alpha=1.0):
            r = x
            k = torch.mm(p, torch.t(r))
            gama = 1.0 / (alpha + torch.mm(r.data, k.data))
            w_tmp = torch.mm(torch.cat((w_bk.grad.data, torch.t(w.grad.data.repeat(1, 1))), 1),
                             torch.t(p.data)) * gama
            p.data /= alpha
            p.sub_(torch.mm(k.data, torch.t(k.data)) * gama.data / alpha)
            w_bk.grad.data = w_tmp[:, :-1]
            w.grad.data = w_tmp[:, -1]
        ...
        for n, w in model.named_parameters():
            ...
            if n == 'layers.fc4.0.bias':
                pro_weight(P4, h_list[0], w, w_bk, alpha=alpha_array[0])

            if n == 'layers.fc5.1.weight':
                w_bk = w

            if n == 'layers.fc5.1.bias':
                pro_weight(P5, h_list[1], w, w_bk,
                           alpha=alpha_array[1])
...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, W4, W5, W6, flag_old, in_layer='fc4'):
    model.train()
    ...
    for iter in range(maxiter):
        ...
        score = model(torch.cat((batch_pos_feats, batch_neg_feats), dim=0), in_layer=in_layer)
        pos_score = score[:batch_pos_feats.size(0), :]
        neg_score = score[batch_pos_feats.size(0):, :]
...
def train_original(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4'):
    model.train()
...
def run_mdnet(img_list, init_bbox, gt=None, seq='seq_name ex)Basketball', savefig_dir='', display=False):
    ...
    P5 = torch.autograd.Variable(torch.eye(512 + 1).type(dtype), volatile=True)*10
    P6 = torch.autograd.Variable(torch.eye(512 + 1).type(dtype), volatile=True)
    ...
    # Initial training
    flag_old = train_owm(model, criterion, init_optimizer, pos_feats, neg_feats, opts['maxiter_init'], P4, P5, P6, W4, W5, W6, flag_old)
    ...
    for i in range(1,len(img_list)):
        ...
        if not success:
            ...
            flag_old = train(model, criterion, update_optimizer, pos_data, neg_data, opts['maxiter_update'], W4, W5, W6, flag_old)
        ...
        # Long term update
            ...
            neg_data = torch.stack(neg_feats_all,0).view(-1,feat_dim)
            flag_old = train_owm(model, criterion, update_optimizer_owm, pos_data, neg_data, opts['maxiter_update'], P4, P5, P6, W4, W5, W6, flag_old)
