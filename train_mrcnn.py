...
def train_mdnet():
    ...
    if pretrain_opts['set_type'] == 'IMAGENET':
        img_home = '/mnt/jgao/jgao/ILSVRC2015/Data/VID/train/'
    ...
    for i in range(pretrain_opts['n_cycles']):
        ...
        for j, k in enumerate(k_list):
            ...
            try:
                for sidx in range(0, len(cropped_scenes)):
                    ...
                    if cur_pos_rois.size(0) == 0 or cur_neg_rois.size(0) == 0:
                        print "______except num rois_______"
                        continue

                    #print "rois %2d, %2d" % \
                    #  (cur_pos_rois.size(0), cur_neg_rois.size(0))
            ...
            print "Cycle %2d, K %2d (%2d), BinLoss %.3f, Prec %.3f, interLoss %.3f, Time %.3f" % \
                      (i, j, k, cls_loss.item(), prec[k], totalInterClassLoss[k], toc)
        ...
        try:
            ...
            total_miou = 0.
        print "Mean Precision: %.3f Triple Loss: %.3f Inter Loss: %.3f IoU: %.3f" % (prec.mean(), cur_score, totalInterClassLoss.mean(),total_miou)
        if cur_score > best_score:
            best_score = cur_score
            if pretrain_opts['use_cpu']:
                model = model.cpu()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-frame_interval", default = 2, type=int, help="frame interval in batch. ex) interval=1 -> [1 2 3 4 5], interval=2 ->[1 3 5]")
